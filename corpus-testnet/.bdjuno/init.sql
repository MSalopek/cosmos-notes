...
CREATE INDEX supply_height_index ON supply (height);/* ---- PARAMS ---- */
...
CREATE INDEX double_sign_evidence_height_index ON double_sign_evidence (height);CREATE TABLE genesis
(
...
CREATE INDEX proposal_validator_status_snapshot_validator_address_index ON proposal_validator_status_snapshot (validator_address);CREATE TABLE modules
(
...
CREATE INDEX slashing_params_height_index ON slashing_params (height);CREATE TABLE fee_grant_allowance
(
    id                 SERIAL      NOT NULL PRIMARY KEY,
